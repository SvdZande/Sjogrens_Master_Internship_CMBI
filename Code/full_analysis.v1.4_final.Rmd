...
#D004_PG <- Read10X("/home/sidneyz/sjogren_data/own_data/SG/D004/filtered_feature_matrix/", strip.suffix = TRUE)
D004_PG <- readRDS("/home/sidneyz/sjogren_data/own_data/SG/D004/filtered_feature_matrix/D004PG.rds")
D004_PG_contigs <- read.csv("/home/sidneyz/sjogren_data/own_data/SG/D004/filtered_feature_matrix/filtered_contig_annotations.csv")
D004_PG_contigs$barcode <- gsub("-1", "", D004_PG_contigs$barcode)
D004_PG_clonotypes <- read.csv("/home/sidneyz/sjogren_data/own_data/SG/D004/filtered_feature_matrix/clonotypes.csv")
...
#D007_PG <- Read10X("/home/sidneyz/sjogren_data/own_data/SG/D007/filtered_feature_matrix/", strip.suffix = TRUE)
D007_PG <- readRDS("/home/sidneyz/sjogren_data/own_data/SG/D007/filtered_feature_matrix/D007.rds")
D007_PG_contigs <- read.csv("/home/sidneyz/sjogren_data/own_data/SG/D007/filtered_feature_matrix/filtered_contig_annotations.csv")
D007_PG_contigs$barcode <- gsub("-1", "", D007_PG_contigs$barcode)
D007_PG_clonotypes <- read.csv("/home/sidneyz/sjogren_data/own_data/SG/D007/filtered_feature_matrix/clonotypes.csv")
...
CITE.df <- read.csv("/home/sidneyz/sjogren_data/conversion_panel_to_protein.csv")
CITE.df$X <- NULL
```
...
write.csv(panel.small, "/home/sidneyz/sjogren_results/own_data/small_antibody_panel.csv")
write.csv(panel.large, "/home/sidneyz/sjogren_results/own_data/large_antibody_panel.csv")
...
autoreactive_cells <- read.csv("/home/sidneyz/sjogren_results/own_data/clonotyping/alldonors_mathijs_threshold2_0405_annotated_seuratmeta.csv")
...
write.csv(seurat_fullmarkers, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/MAST_0.3cluster_markers_v1.4b_RNA.csv")
seurat_fullmarkers <- read.csv("/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/MAST_0.3cluster_markers_v1.4b_RNA.csv")
```
...
#If needed, you can also load the seurat objects I generated in this script from here:
...
subsetted_contigs <- read.csv("/home/sidneyz/sjogren_results/own_data/clonotyping/clonotypes_ownscript_mathijs_threshold2_0405_annotated.csv")
subsetted_contigs <- read.csv("/home/sidneyz/sjogren_results/own_data/clonotyping/alldonors_mathijs_threshold2_0405_annotated.csv")
...
#Subset easier to look at tables
...
#Do the same for the CITE-seq data
...
#Save the result
write.csv(seurat_markers, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/sjogren_v1.4b_markers_MAST0.7_RNA.csv")
write.csv(fullpanel_markers, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/sjogren_v1.4b_markers_MAST0.7_CITE_fullpanel.csv")

seurat_markers <- read.csv("/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/sjogren_v1.4b_markers_MAST0.7_RNA.csv")
rownames(seurat_markers) <- seurat_markers$X
seurat_markers$X <- NULL
fullpanel_markers <-read.csv("/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/sjogren_v1.4b_markers_MAST0.7_CITE_fullpanel.csv")
...
#Make a vector of colors for the cluster annotations
...
#Make plots at the l1 and l2 level
...
#Split annotation plots based on tissue
...
fig
htmlwidgets::saveWidget(as_widget(fig), "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/UMAP_3D_sjogren_v1.4b_subset.html")
```

```{r Compare IGHM to IGHG per cluster}
#IGHM vs IGHG comparisons per cluster

#Make a column that contains the cluster and isotype
...
for (i in unique(seurat_combined_subset@meta.data$annotation)){
  groups_to_get.IGHM <- paste0(i, "_IGHM") #all IGM cells in that cluster
  groups_to_get.IGHG <- paste0(i, "_IGHG") # all IGG cells in that cluster
  no_cells <- F
  
  #try to find the markers, but if one group contains less than 3 cells, skip the comparison
...
#Save the result
write.csv(IGHM_vs_IGHG.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/IGHM_vs_IGHG_v1.4b_MAST0.7_RNA.csv")
IGHM_vs_IGHG.df <- read.csv("/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/IGHM_vs_IGHG_v1.4b_MAST0.7_RNA.csv")

#Do the same for protein markers
...
write.csv(IGHM_vs_IGHG.cite.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/IGHM_vs_IGHG_v1.4b_MAST0.7_CITE.csv")
```
...
#Join with easier to read protein names
CITE.df.2 <- CITE.df
colnames(CITE.df.2) <- c("gene", "protein")
...
#Make an annotation column based on the cluster and autoreactivity group
...
#Find markers of RF vs normal, ANA vs normal and RF+ANA vs normal
for (i in unique(annotation)){
  ...
  #if there are more than 3 RF cells in the cluster
  if (rf_length >= 3){
    ...
    #if there are more than 3 RF cells in the cluster, and these are all from a donor with a full CITE-seq panel, run a protein analysis:
...
write.csv(rf.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/rf_cluster_v1.4b_MAST0.7_RNA.csv")
...
write.csv(rf.cite.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/rf_cluster_v1.4b_MAST0.7_CITE.csv")
...
write.csv(ana.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/ana_cluster_v1.4b_MAST0.7_RNA.csv")
...
write.csv(ana.cite.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/ana_cluster_v1.4b_MAST0.7_CITE.csv")
...
write.csv(all.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/all_cluster_v1.4b_MAST0.7_RNA.csv")
...
write.csv(all.cite.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/all_cluster_v1.4b_MAST0.7_CITE.csv")
```
...
plot_clone <- function(seurat, clone_number){
  ...
  #Make a metadata column that states whether a cell is part of the clone or not
  ...
  #Highlight that clonotype on a UMAP
...
#Show normalised protein expressions in a violin plot, per cluster
Idents(seurat_fullpanel) <- seurat_fullpanel@meta.data$annotation
...
#Do the same as above but now also split by isotype

#Make an annotation that contains the cluster and isotype
...
seurat_fullpanel_filter <- subset(seurat_fullpanel, subset = isotype %in% c("IGHG", "IGHM")) #only keep IGHM and IGHG entries
...
#Make an annotation of the tissue and cluster
...
#Calculate the average expression per cluster, epr tissue
...
#If a donor name is in the cluster name, take it out
average$cluster <- gsub("\\(B007\\)", "-B007-", average$cluster)
average$cluster <- gsub("\\s*\\([^\\)]+\\)", "", average$cluster)
average$cluster <- gsub("-B007-","\\(B007\\)", average$cluster)

#Convert to long format
average <- average[rep(seq_len(nrow(average)), each = 2), ] #duplicate each row
average$gene <- rep(c("MKI67", "AICDA"), times = nrow(average)/2) #add the genes
odd_rows <- seq_len(nrow(average)) %%2 #define what odd rows are (MKI67 rows)
average$expression <- NA
average$expression[odd_rows == 1] <- average$MKI67[odd_rows == 1] #make it such that each row corresponds to one gene
...
#remove the tissue name from the cluster name
...
#Subset cells that express MKI67, AICDA, or both
...
#Set non-autoreactive cells to "none"
...
#Make a plot of how many cells express MKI67, AICDA or both
...
#Make a table of how many cells express MKI67, AICDA or both
...
#which clusters consist of which tissues?
...
```{r Plot the amount of autoreactive clones per cluster, tissue}
...
#For plotting purposes: when a cluster has only one type of autoreactive cell, add an entry for the other with the amount of cells set to NA
...
for (i in 1:nrow(b_clusters_dup)){
  if(b_clusters_dup$n[[i]] == 1){ #if there is only one autoreactive group for that tissue and annotation
    investigation_cluster <- b_clusters[b_clusters$tissue %in% b_clusters_dup$tissue[[i]] & b_clusters$annotation %in% b_clusters_dup$annotation[[i]],] #take that row out
    
    second_row <- investigation_cluster
    second_row$n <- NA
    
    #make a second entry with the amount of autoreactive cells set to NA
...
#Add the cluster annotation to the list of markers
annot$cluster <- rownames(annot)
seurat_markers<- inner_join(seurat_markers, annot, by = "cluster")

write.csv(seurat_markers, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/MAST_0.6clustermarkers_seurat_v1.4b.RNAannot.csv")

#split into upregulated and downregulated markers
...
#Add an annotation based on the cluster + antigen (or isotype if non-autoreactive)
seurat_combined_subset <- sjogren::add_dotplot_annot(seurat_combined_subset, mode = "cluster", include_IGHA_IGHD = F)

#Get the marker annotations out
...
#Make sure that the annotations are in the correct order
sizes <- sizes[order(match(sizes$annotation,group_order)),]

#remove IGHD/IGHA cells
...
#Add the gene annotation sidebar
...
subsetted_contigs.2 <- read.csv("/home/sidneyz/sjogren_results/own_data/clonotyping/clonotypes_B005B007_mathijs_threshold2_0405_annotated.csv")
...
#Subset based on donor (D00 or B00)
...
DefaultAssay(gc_B00) <- "RNA" 
plot.small <- FeaturePlot(gc_B00, features = c("BCL6", "CXCR5", "CD27", "IGHD", "MKI67", "AICDA", "LMO2", "ITGAX"), 
                          order = T, slot = "data", combine = F)
color_scale <- scale_colour_gradientn(colours = viridis(n = 11))
plot.small <- lapply(plot.small, function(x) x + color_scale)
plot.small <- wrap_plots(plot.small)

plot_small_gc_B00 <- plot.small
...
```{r Germinal centre reclustering (D00)}
###D00 RUN
...
plot.small <- FeaturePlot(gc_D00, features = c("BCL6", "CXCR5", "CD27", "IGHD", "MKI67", "AICDA", "LMO2", "ITGAX"), 
                          order = T, slot = "data", combine = F)
color_scale <- scale_colour_gradientn(colours = viridis(n = 11))
plot.small <- lapply(plot.small, function(x) x + color_scale)
plot.small <- wrap_plots(plot.small)
plot_small_gc_D00 <- plot.small
...
write.csv(annotations.df, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/recluster_analyses/gc_recluster_donorsep.csv")
```
...
#Only keep salivary gland MBC cells, separate by donor
...
plot.small <- FeaturePlot(sg_B00, features = c("BCL6", "CD27", "CXCR5", "IGHM", "CCR7", "MKI67"), order = T, combine = F)
color_scale <- scale_colour_gradientn(colours = viridis(n = 11))
plot.small <- lapply(plot.small, function(x) x + color_scale)
plot.small <- wrap_plots(plot.small)

plot_small_sg_B00 <- plot.small
...
plot.small <- FeaturePlot(sg_D00, features = c("BCL6", "CD27", "CXCR5", "IGHM", "CCR7", "MKI67"), order = T, combine = F)
color_scale <- scale_colour_gradientn(colours = viridis(n = 11))
plot.small <- lapply(plot.small, function(x) x + color_scale)
plot.small <- wrap_plots(plot.small)

plot_small_sg_D00 <- plot.small
...
##Lymph node MBC recluster
```{r LN MBC recluster}
ln_mbc <- subset(seurat_combined_subset, tissue == "Lymph node")
ln_mbc <- subset(ln_mbc, annotation %in% c("FCRL4+ MBC",  "IGHG+/IGHA+ MBC", "IGHA+ MBC"))
ln_mbc <- subset(ln_mbc, donor %in% c("B007", "B005"), invert = T)

DefaultAssay(ln_mbc) <- "RNA"
all.genes <- rownames(ln_mbc)
ln_mbc <- ScaleData(ln_mbc, features = all.genes, verbose = FALSE)
ln_mbc <- ScaleData(ln_mbc, features = rownames(ln_mbc[["CITE"]]), assay = "CITE", verbose = FALSE)


ln_mbc <- FindVariableFeatures(ln_mbc, selection.method = "vst", nfeatures = 3000)
VariableFeatures(ln_mbc[["CITE"]]) <- rownames(seurat_list_filtered[["D002"]][["CITE"]])

#Run ICA
ln_mbc <- RunICA(ln_mbc, features = VariableFeatures(object = ln_mbc), verbose = FALSE)

ica <- ln_mbc@reductions$ica
rownames(ica@cell.embeddings) <- colnames(ln_mbc[["RNA"]])
ln_mbc@reductions$ica <- ica

#Reclustering
ln_mbc <- RunUMAP(ln_mbc, reduction =  "ica", dims = 1:40)

ln_mbc <- FindNeighbors(ln_mbc, reduction = "ica", dims = 1:40)

ln_mbc <- FindClusters(ln_mbc, resolution =  seq(from = 0.1, to = 0.9, by = 0.1), algorithm = 3)
clust_seurat <- ln_mbc@meta.data %>% dplyr::select(dplyr::contains("RNA_snn_res."))
clustree(clust_seurat, prefix="RNA_snn_res.")

#Now, set the proper resolution:
ln_mbc <- FindClusters(ln_mbc, resolution = 0.6, algorithm = 3)


#UMAP plotting
n <-length(levels(ln_mbc))
qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
cols = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
pie(rep(1,n), col=sample(cols, n))
clust_cols <- cols[1:n]
...
umap <- DimPlot(ln_mbc, reduction = 'umap', label = TRUE, repel = TRUE, label.size = 4, pt.size = 1) + NoLegend()
umap_donor <- DimPlot(ln_mbc, reduction = 'umap', label = F, repel = TRUE, label.size = 2.5, pt.size = 1, group.by = "donor") 
umap_reactivity <- DimPlot(ln_mbc, reduction = 'umap', label = TRUE, repel = TRUE, label.size = 2.5, pt.size = 1, group.by = "reactivity") + NoLegend()
umap_tissue <- DimPlot(ln_mbc, reduction = 'umap', label = F, repel = TRUE, label.size = 2.5, pt.size = 1, group.by = "tissue")


ln_mbc@meta.data$antigen[is.na(ln_mbc@meta.data$antigen)] <- "NA"
ln_mbc@meta.data$group[is.na(ln_mbc@meta.data$group)] <- "NA"

umap_antigen <- DimPlot(ln_mbc, reduction = 'umap', label = FALSE, repel = TRUE, label.size = 2.5, na.value = element_blank(),
                        pt.size = 1, group.by = "antigen", order = c("RF,Ro60,Ro52,La", "RF,Ro60,La", "RF,Ro60","Ro60,Ro52,La", "Ro60,Ro52","Ro60,La","Ro52,La","La","Ro60","Ro52","RF","NA"), cols = c("grey","hotpink","purple","darkblue","cyan" ,"green","yellow","orange","red", "darkgreen", "brown", "darkcyan"))
umap_group <- DimPlot(ln_mbc, reduction = 'umap', label = FALSE, repel = TRUE, label.size = 2.5, group.by = "group", pt.size = 1, order = c("RF","ANA", "NA"), cols = c("grey",  "red", "blue"))

ln_mbc@meta.data$antigen[ln_mbc@meta.data$antigen == "NA"] <- NA
ln_mbc@meta.data$group[ln_mbc@meta.data$group == "NA"] <- NA

plot <- FeaturePlot(ln_mbc, features = c("CXCR5", "CD27", "MKI67","FCRL4", "IGHM", "IGHG1", "IGHA1", "IGHD"), order = T)
plot.cite <- FeaturePlot(ln_mbc, features = c("Hu.CD40", "Hu.CD25", "Hu.CD86", "Hu.CD196", "Hu.HLA.ABC", "Hu.HLA.DR"), order = T)
...
#Plot the donor contribution
ggplot(ln_mbc@meta.data, aes(x = seurat_clusters,  fill = donor)) + geom_bar(position = "fill") + coord_flip()  + xlab("cluster") + ylab("fraction")

#Add an isotype
ln_mbc@meta.data$chain <- NA
ln_mbc@meta.data$chain[ln_mbc@meta.data$c_gene == "IGHM"] <- "IGHM"
ln_mbc@meta.data$chain[ln_mbc@meta.data$c_gene %in% c("IGHG1", "IGHG2", "IGHG3", "IGHG4")] <- "IGHG"

#Find markers per isotype
Idents(ln_mbc) <- ln_mbc@meta.data$chain
isotype_markers <- FindMarkers(ln_mbc, ident.1 = "IGHM", ident.2 = "IGHG", only.pos = FALSE, min.pct = 0.25, logfc.threshold = 0.25, test.use = "MAST", slot = "data", assay = "RNA")
isotype_markers <- subset(isotype_markers, isotype_markers$p_val_adj <= 0.05)

seurat_fullpanel <- subset(ln_mbc, donor %in% c("D003", "D004", "D007", "D001+D002+D003+D004"))

isotype_markers.cite <- FindMarkers(seurat_fullpanel, ident.1 = "IGHM", ident.2 = "IGHG", only.pos = FALSE, min.pct = 0.25, logfc.threshold = 0.25, test.use = "wilcox", slot = "data", assay = "CITE")
isotype_markers.cite <- subset(isotype_markers.cite, isotype_markers.cite$p_val_adj <= 0.05)
isotype_markers.cite$gene <- rownames(isotype_markers.cite)
isotype_markers.cite <- left_join(isotype_markers.cite, CITE.df.2)



#Compare the expression of one specific gene/protein
ln_mbc_fullpanel <- subset(ln_mbc, donor %in% c("D003", "D004", "D007", "AIM"))
IGHM_counts <- subset(ln_mbc_fullpanel, chain == "IGHM")
IGHM_counts <- IGHM_counts[["CITE"]]@counts
IGHG_counts <- subset(ln_mbc_fullpanel, chain == "IGHG")
IGHG_counts <- IGHG_counts[["CITE"]]@counts

CD40_IGHM <- IGHM_counts[rownames(IGHM_counts) == "Hu.CD40",]
CD40_IGHG <- IGHG_counts[rownames(IGHG_counts) == "Hu.CD40",]

wilcox.test(CD40_IGHM, CD40_IGHG)

mean(CD40_IGHM)
mean(CD40_IGHG)

CD86_IGHM <- IGHM_counts[rownames(IGHM_counts) == "Hu.CD86",]
CD86_IGHG <- IGHG_counts[rownames(IGHG_counts) == "Hu.CD86",]

wilcox.test(CD86_IGHM, CD86_IGHG)

mean(CD86_IGHM)
mean(CD86_IGHG)

IGHM_counts_rna <- subset(ln_mbc, chain == "IGHM")
IGHM_counts_rna <- IGHM_counts_rna[["RNA"]]@counts
IGHG_counts_rna <- subset(ln_mbc, chain == "IGHG")
IGHG_counts_rna <- IGHG_counts_rna[["RNA"]]@counts

CD40_IGHM_rna <- IGHM_counts_rna[rownames(IGHM_counts_rna) == "CD40",]
CD40_IGHG_rna <- IGHG_counts_rna[rownames(IGHG_counts_rna) == "CD40",]

wilcox.test(CD40_IGHM_rna, CD40_IGHG_rna)

mean(CD40_IGHM_rna)
mean(CD40_IGHG_rna)

CD86_IGHM_rna <- IGHM_counts_rna[rownames(IGHM_counts_rna) == "CD86",]
CD86_IGHG_rna <- IGHG_counts_rna[rownames(IGHG_counts_rna) == "CD86",]

wilcox.test(CD86_IGHM_rna, CD86_IGHG_rna)

mean(CD86_IGHM_rna)
mean(CD86_IGHG_rna)

CD80_IGHM_rna <- IGHM_counts_rna[rownames(IGHM_counts_rna) == "CD80",]
CD80_IGHG_rna <- IGHG_counts_rna[rownames(IGHG_counts_rna) == "CD80",]

wilcox.test(CD80_IGHM_rna, CD80_IGHG_rna)
...
##FCRL4 analyses
```{r FCRL4 sub-analyses}
#FCRL4 sub-analyses
#Investigate the variable gene usage
contig_all_list <- list(D001_LN_contigs, D002_LN_contigs, D003_LN_contigs, D004_LN_contigs, D004_PG_contigs, D007_PG_contigs,
                    B005_1_contigs, B005_2_contigs, B007_1_contigs, B007_2_contigs, pooled_PBMC_contigs)
contig_all <- rbindlist(contig_all_list, fill = TRUE)

contig_all$cell_barcode <- paste0(contig_all$ident, "_", contig_all$barcode)

#Subset FCRL4+ MBCs, SG MBCs and LN MBCs
fcrl4_cluster <- subset(seurat_combined_subset, annotation == "FCRL4+ MBC")
memB_cluster <- subset(seurat_combined_subset, annotation %in% c("IGHA+ MBC", "IGHG+/IGHA+ MBC"))
SG_memB_cluster <- subset(seurat_combined_subset, annotation %in% c("Activated SG MBC", "Exhausted SG MBC","CXCR4+ SG MBC"))

#Add the cluster annotation to the contig data
autoreactive_cells$cell_barcode <- rownames(autoreactive_cells)
contig_all <- left_join(contig_all, autoreactive_cells, by = "cell_barcode")
contig_all$annotation <- NA
contig_all$annotation[contig_all$cell_barcode %in% colnames(fcrl4_cluster)] <- "FCRL4"
contig_all$annotation[contig_all$cell_barcode %in% colnames(memB_cluster)] <- "LN MBC"
contig_all$annotation[contig_all$cell_barcode %in% colnames(SG_memB_cluster)] <- "SG MBC"

#Summarize by cluster and variable gene
sum <- contig_all %>%
  group_by(annotation, v_gene) %>%
  filter(v_gene != "none" & !is.na(annotation)) %>%
  dplyr::count()

colors = c("orange", "darkcyan", "purple")

sum <- sum[sum$n > 5,] #only keep genes that are used in more than 5 cells

#Plot the result
ighv_plot <- ggplot(sum, aes(x=reorder(v_gene, -n), y = n, fill = annotation)) + geom_col(position = position_dodge(0.5), width = 0.35) +
  ...
  xlab("Variable gene call") + ylab("Count") + scale_y_continuous(expand=c(0,0)) +
  guides(fill=guide_legend(title="annotation")) + scale_fill_manual(values = colors)

#To which clusters do FCRL4+ cells map?
fcrl4 <- subset(seurat_combined_subset, FCRL4 > 0) #keep cells that express FCRL4, regardless of cluster
test <- fcrl4@meta.data %>%
  group_by(annotation) %>%
  count()
ggplot(test, aes(x=annotation, y = n, fill= annotation)) + geom_col(position = "dodge")

#Compare clone sizes between FCRL4 MBC and LN MBC
clones_fcrl4 <- fcrl4_cluster@meta.data$clonotype_new
clones_memB <- memB_cluster@meta.data$clonotype_new

clones_fcrl4 <- fcrl4_cluster@meta.data[fcrl4_cluster@meta.data$clonotype_new %in% clones_fcrl4,] #take out all clonotypes of which a cell is present in the FCRL4 cluster
clones_memB <- memB_cluster@meta.data[memB_cluster@meta.data$clonotype_new %in% clones_memB,] #do the same for the LN MBC clusters
clones_fcrl4 <- clones_fcrl4[!is.na(clones_fcrl4$clonotype_new),]
clones_memB <- clones_memB[!is.na(clones_memB$clonotype_new),]

clono_fcrl4 <- clones_fcrl4 %>%
  group_by(clonotype_new) %>%
  dplyr::count()

clono_memB <- clones_memB %>%
  group_by(clonotype_new) %>%
  dplyr::count()

wilcox.test(clono_memB$n, clono_fcrl4$n) #compare the clonotype sizes

#Plot the result
clono_fcrl4$cluster <- "FCRL4+ MBC"
clono_memB$cluster <- "LN MBC"

clono <- rbind(clono_fcrl4, clono_memB)

ggplot(clono, aes(x = cluster, y = n, fill = cluster)) + geom_boxplot() + geom_jitter(alpha = 0.4, aes(col = cluster))

#Compare DEGs FCRL4 cluster to LN MBC
fcrl4_deg <- FindMarkers(seurat_combined_subset, ident.1 = "FCRL4+ MBC", ident.2 = c("IGHA+ MBC", "IGHG+/IGHA+ MBC"), assay = "RNA", slot = "data", test.use = "wilcox",
                         min.pct = 0.25)
fcrl4_deg <- fcrl4_deg[fcrl4_deg$p_val_adj <= 0.05,]
fcrl4_deg$gene <- rownames(fcrl4_deg)
fcrl4_deg <- fcrl4_deg[order(-fcrl4_deg$avg_log2FC),]

write.csv(fcrl4_deg, "/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/recluster_analyses/fcrl4_deg_vs_ln_mbc.csv")

fcrl4_deg_pos <- fcrl4_deg[fcrl4_deg$avg_log2FC > 0,]
fcrl4_deg_neg <- fcrl4_deg[fcrl4_deg$avg_log2FC < 0,]

go_pos <- enrichGO(fcrl4_deg_pos$gene, ont = "BP", OrgDb = "org.Hs.eg.db", keyType = "SYMBOL", pvalueCutoff = 0.05, pAdjustMethod = "BH")
go_neg <- enrichGO(fcrl4_deg_neg$gene, ont = "BP", OrgDb = "org.Hs.eg.db", keyType = "SYMBOL", pvalueCutoff = 0.05, pAdjustMethod = "BH")
dotplot(go_pos) + ggtitle("Pathways upregulated in FCRL4+ MBC cluster over LN MBC")
dotplot(go_neg) + ggtitle("Pathways downregulated in FCRL4+ MBC cluster over LN MBC")
```

##Figures
```{r Figure 1}
#Figure 1
#Make the UMAP containing the cluster annotations
n <-length(levels(seurat_combined_subset))
qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
cols = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
pie(rep(1,n), col=sample(cols, n))
...
#Make the heatmap of DEGs per cluster
clust_cols <- distinctColorPalette(k=length(unique(seurat_combined_subset@meta.data$annotation)) + 4)
sce <- as.SingleCellExperiment(seurat_combined_subset, assay = "RNA")
celltype_mean <- scuttle::aggregateAcrossCells(sce, ids = sce$annotation, statistics = "mean",
                                               use.assay.type = 'scaledata')
...
#Plot the tissue contribution per cluster
...
#Convert heatmap to ggplot for easier merging with other figures
heatmap_cluster_RNA_plot <- as.ggplot(heatmap_cluster_RNA_plot)

#Make a patchwork object
patch <- (umap_annotation_plot + heatmap_cluster_RNA_plot)/ (tissue_bar_plot + boxplot_clono)

#Save figure
png(file="/home/sidneyz/sjogren_results/figures/figure_1.png", width=1600, height=1800)

patch + plot_annotation(tag_levels = "A") & 
  theme(plot.tag = element_text(size = 22, face = "bold")) #add tags
dev.off()
```

```{r Figure 2}
##FIGURE 2
#Count how many autoreactive cells there are in each cluster
b_clusters <- seurat_combined_subset@meta.data %>%
  group_by(tissue, annotation, group) %>%
  filter(group != "none" & group != "both" & !is.na(group) & group != "NA") %>%
  dplyr::count()

#For plotting purposes: if a tissue+cluster only has one type of autoreactive cell, add the other with a count of NA
...
#Subset the three tissues
blood <- b_clusters[b_clusters$tissue == "Blood",]
sal <- b_clusters[b_clusters$tissue == "Salivary gland",]
ln <- b_clusters[b_clusters$tissue == "Lymph node",]

#Make an overall plot (but this contains each cluster in each tissue, which is not correct)
fig2A <- ggplot(b_clusters, aes(x = annotation, y = n, fill = group)) + geom_col(position = "dodge") +
  ...
  xlab("Cluster") + ylab("Number of cells") + scale_y_continuous(expand=c(0,0), limits = c(0, 250)) + guides(fill=guide_legend(title="antigenicity")) +
  facet_grid(~tissue) + scale_fill_manual(values = c("grey", "black"))

legend <- get_legend(fig2A) #save the legend

#Plot for each tissue separately:
fig2A_blood <- ggplot(blood, aes(x = annotation, y = n, fill = group)) + geom_col(position = "dodge") +
  theme_bw() +  theme(panel.grid.major.x = element_blank(),panel.border = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1), text=element_text(size = 22), legend.position = "none") +
  xlab("Cluster") + ylab("Number of cells") + scale_y_continuous(expand=c(0,0), limits = c(0, 250)) + guides(fill=guide_legend(title="antigenicity")) +
  facet_grid(~tissue) + scale_fill_manual(values = c("grey", "black"))

fig2A_sal <- ggplot(sal, aes(x = annotation, y = n, fill = group)) + geom_col(position = "dodge") +
  theme_bw() +  theme(panel.grid.major.x = element_blank(),panel.border = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1), text=element_text(size = 22), legend.position = "none") +
  xlab("Cluster") + ylab("Count") + scale_y_continuous(expand=c(0,0), limits = c(0, 250)) + guides(fill=guide_legend(title="antigenicity")) +
  facet_grid(~tissue) + scale_fill_manual(values = c("grey", "black"))

fig2A_ln <- ggplot(ln, aes(x = annotation, y = n, fill = group)) + geom_col(position = "dodge") +
  theme_bw() +  theme(panel.grid.major.x = element_blank(),panel.border = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1), text=element_text(size = 22), legend.position = "none") +
  xlab("Cluster") + ylab("Count") + scale_y_continuous(expand=c(0,0), limits = c(0, 250)) + guides(fill=guide_legend(title="antigenicity")) +
  facet_grid(~tissue) + scale_fill_manual(values = c("grey", "black"))

#Combine the tissue plots, add the legend, and make them share a y axis
fig2A <- plot_grid((fig2A_blood + theme(axis.title.x = element_blank())), (fig2A_ln + theme(axis.text.y = element_blank(),
                                                 axis.ticks.y = element_blank(),
                                                 axis.title.y = element_blank() )), 
                   (fig2A_sal +theme(axis.text.y = element_blank(),
                                     axis.ticks.y = element_blank(),
                                     axis.title.y = element_blank(), axis.title.x = element_blank() )), legend, nrow = 1, rel_widths = c(2,2,2,1), align = "hv", axis ="tblr")

#Compare clonotype sizes RF vs ANA
clono.meta <- seurat_combined_subset@meta.data %>%
...
#Make plot of sizes of clones as percentage of total of that donor
...
#Make patchwork object
patch2 <- fig2A/(fig2B+ fig2C)

#Save figure
png(file="/home/sidneyz/sjogren_results/figures/figure_2.png", width=1100, height = 900)
...
```{r Figure 3}
##FIGURE 3
#Make larger featureplots for supplemental
...
fig3A.small <- plot_small_gc_B00
fig3B.small <- plot_small_gc_D00

#Add labels
fig3A<- plot_grid(fig3A, labels = c("A"), label_size = 22)
fig3B<- plot_grid(fig3B, labels = c("B"), label_size = 22)

fig3A.small <- plot_grid(fig3A.small, labels = c("A"), label_size = 22)
fig3B.small <- plot_grid(fig3B.small, labels = c("B"), label_size = 22)

#Import tonsil-derived figures
fig3C <- readRDS("/home/sidneyz/sjogren_results/figures/UMAP_tonsil_annotated.RDS")
fig3D <- readRDS("/home/sidneyz/sjogren_results/figures/gc_mapping_tonsil.RDS")

#Adjust font and margin sizes
fig3C <- fig3C +  guides(color = guide_legend(override.aes = list(size=9))) + labs(tag = "C")  + theme(plot.margin = margin(t = 0,  # Top margin
                                                                                                         r = 0,  # Right margin
                                                                                                         b = 0,  # Bottom margin
                                                                                                         l = 0), # Left margin) 
                                                                                                       strip.text = element_text(size = 22), text=element_text(size=22))
...
#Save figures
png(file="/home/sidneyz/sjogren_results/figures/figure_3.png", width=1400, height = 2400)
fig3A.small/fig3B.small/fig3C/fig3D
dev.off()

png(file="/home/sidneyz/sjogren_results/figures/figure_3_supplemental.png", width=1400, height = 1600)
fig3A/fig3B
dev.off()
```
...
```{r Figure 4}
##FIGURE 4
#Same as figure 3 but now for SG MBCs
...
fig4A.small <- plot_small_sg_B00
fig4B.small <- plot_small_sg_D00

fig4A.small <- plot_grid(fig4A.small, labels = c("A"), label_size = 22)
fig4B.small <- plot_grid(fig4B.small, labels = c("B"), label_size = 22)

#Subset LN MBCs
ln_mbc <- subset(seurat_combined_subset, tissue == "Lymph node")
ln_mbc <- subset(ln_mbc, annotation %in% c("FCRL4+ MBC", "IGHA+ MBC", "IGHG+/IGHA+ MBC"))

#Compare clonotype sizes
...
#Plot GO analysis of RF SG MBC markers compared to non-auto SG MBC
fig4D <- dotplot(go, showCategory= 9, font.size = 20) + labs(tag ="D") + theme(text=element_text(size=20))

#Save figures
png(file="/home/sidneyz/sjogren_results/figures/figure_4.png", width=1400, height = 2000)
fig4A.small/fig4B.small/(fig4C+fig4D)
dev.off()

png(file="/home/sidneyz/sjogren_results/figures/figure_4_supplemental.png", width=1400, height = 1600)
fig4A/fig4B
dev.off()
...
fig4E <- readRDS("/home/sidneyz/sjogren_results/own_data/seurat_v1.4b/recluster_analyses/sg_mbc_cellchat.RDS")

#Capture the heatmap outputs
fig4E.1 <- grid.grabExpr(draw(fig4E[[1]]))
fig4E.2 <- grid.grabExpr(draw(fig4E[[2]]))
fig4E <- plot_grid(fig4E.1, fig4E.2)

#Save figure
png(file="/home/sidneyz/sjogren_results/figures/supplemental_figure_cellchat.png", width=1500, height = 900)
fig4E
dev.off()
```
