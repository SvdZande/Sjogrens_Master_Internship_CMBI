...
#Define a faster version of the %in% operator
  ...
  #Only keep productive chains (if not filtered yet)
  contig <- contig[contig$cdr3_nt != "None" & contig$productive == "True",]


  #make sure threshold is okay: not larger than the amount of nucleotides in the shortest sequence, not below zero, and has to be a number
  ...
  print(paste0(length(dual_chains), " cells with two IGH chains were found.")) #print a message saying how many dual IGH cells were found
  ...
  contig <- left_join(contig, contig_IGH_grouping, by = c("v_gene", "d_gene", "j_gene")) #add the IGH clonotypes to the cells based on their IGH genes
...
##Clonotyping with test and real data
...
contig.copy <- contig_full #make a backup

#Threshold of 2 nucleotides
contig <- sjogren::clonotype(contig, threshold = 2, levenshtein = F)
write.csv(contig, "sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold2_0405.csv")


contig <- contig.copy #reset to original contig data frame

#Threshold of 3 nucleotides
contig <- sjogren::clonotype(contig, threshold = 3, levenshtein = F)
write.csv(contig, "sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold3_0405.csv")
...
contig <- sjogren::clonotype(contig, threshold = 4, levenshtein = F)
write.csv(contig, "sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold4_0405.csv")
...
#Threshold of 2 nt
contig_0507 <- sjogren::clonotype(contig_0507, threshold = 2, levenshtein = F)
write.csv(contig_0507, "sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold2_0405.csv")


contig_0507 <- contig #reset back to original df

#Threshold 3
contig_0507 <- sjogren::clonotype(contig_0507, threshold = 3, levenshtein = F)
write.csv(contig_0507, "sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold3_0405.csv")

contig_0507 <- contig

#Threshold 4
contig_0507 <- sjogren::clonotype(contig_0507, threshold = 4, levenshtein = F)
write.csv(contig_0507, "sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold4_0405.csv")
...
contig_full_02 <- read.csv("sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold2_0405.csv")
contig_full_03 <- read.csv("sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold3_0405.csv")
contig_full_04 <- read.csv("sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold4_0405.csv")

contig_b0057_02 <-  read.csv("sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold2_0405.csv")
contig_b0057_03 <-  read.csv("sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold3_0405.csv")
contig_b0057_04 <-  read.csv("sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold4_0405.csv")
...
#D00*
contig_full <- read.csv("sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold2_0405.csv")
contig_full$clonotype_new <- as.character(contig_full$clonotype_new)
contig_full$X <- NULL

#B00*
contig_0507 <- read.csv("sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold2_0405.csv")
...
write.csv(contig_full, "sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold2_0405_annotated.csv")
write.csv(contig_0507, "sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold2_0405_annotated.csv")

contig_full <- read.csv("sjogren_results/own_data/combined/clonotypes_ownscript_mathijs_threshold2_0405_annotated.csv")
contig_0507 <- read.csv("sjogren_results/own_data/combined/clonotypes_B005B007_mathijs_threshold2_0405_annotated.csv")
```
...
max_D00 <- max(as.integer(contig_full$clonotype_new[contig_full$clonotype_new != "none"]))
contig_0507$clonotype_new[contig_0507$clonotype_new != "none"] <- as.character(as.integer(contig_0507$clonotype_new[contig_0507$clonotype_new != "none"]) + max_D00)

colnames(contig_full)[!(colnames(contig_full) %in% colnames(contig_0507))]
contig_all <- rbindlist(list(contig_full, contig_0507), fill = TRUE)
write.csv(contig_all, "sjogren_results/own_data/combined/alldonors_mathijs_threshold2_0405_annotated.csv")
```
...
contig_seurat$clonotype_new[contig_seurat$clonotype_new == "none"] <- NA #if the clonotype cannot be assigned, set to NA
...
write.csv(contig_seurat, "sjogren_results/own_data/combined/alldonors_mathijs_threshold2_0405_annotated_seuratmeta.csv")
```
...
#Clonal overlap D004
D004 <- contig_full_02[contig_full_02$donor == "D004",]
